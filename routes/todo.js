...
router.post("/add", requestMade, function(req,res){
...
router.post("/:itemID/complete", requestMade, function(req, res){
  models.todos.findById(req.params.itemID).then((task) => {
    task.update({complete: true}).then(() => {res.redirect("/todo");})
  });
});

router.post("/completed/delete", requestMade, function(req,res){
